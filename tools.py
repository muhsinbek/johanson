...
def translate(text):
    ...
    return text.replace(
        ...
        .replace('k'.replace(u'\ufe0f', u''), 'k') \
        .replace('k'.replace(u'\ufe0f', u''), 'o') \
        .replace('✝️', '') \
        .replace('k'.replace(u'\ufe0f', u''), '')
...
def scaner(text):
    ...
    is_bad = False
    bad_words = ["kot", "chichqoq", "jalab", "xaromi", "xoromi", "qotoq", "qutoq"]
    good_words = ["kotan", "kotanik", "botqa", "hisobot", "astrobot", "yunusobot", "botiq", "botmon", "robotexnika",
                 "botma", "botanika", "botqo", "botgan", "botkan", "botib", "botir", "botak", "botmon", "razrabot",
                 "obrabot", "robot", "isbot", "rabot", "umrbot", "botstrap", "boot", "bootcamp", "kotta", "kotik", 
                 "boykot", "kotton"]
    ...
    for i in low.split():
        if len(i) == 1:
            if i != "k" or i != "o" or i != "t":
                low = low.replace(i, '')
        else:
            if 'k' not in i and 'o' not in i and 't' not in i:
                low = low.replace(i, '')
    ...
    low = low.replace('k ot', 'kot').replace('ko t', 'kot').replace('k o t', 'kot')
