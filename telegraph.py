...
def node(message):
    ...
    msg = 'https://t.me/kiuf_offtop/{}'.format(message.message_id)
        ...
        if url:
            ...
            try:
                page = client.edit_page(path=data['url'][18:], title='Barcha kod va izohlar', content=result[:-1],
                                        author_name='Johanson', author_url='https://t.me/KIUFrobot')
            except Exception as e:
                log(7, e)
        else:
            log(6, 'started to create')
            page = client.create_page(title='Barcha kod va izohlar', content=result, author_name='Johanson',
                                      author_url='https://t.me/KIUFrobot')
            data['url'] = page.url
            log(2, data)
...
def update(message, is_code=False):
    ...
    result[1]['children'] = [
        '{likes}  {like} {space} {peoples}  {people} {space} {comments}  {comment}'.format(like=icons[0],
                                                                                           ...
                                                                                           space=" " * 10)]
    if info['url']:
        page = client.edit_page(path=info['url'][18:], title='Kod uchun izohlar', content=result, author_name='Johanson',
                                author_url='https://t.me/KIUFrobot', return_content=True)
    else:
        page = client.create_page(title='Kod uchun izohlar', content=result, author_name='Johanson',
                                  author_url='https://t.me/KIUFrobot')
